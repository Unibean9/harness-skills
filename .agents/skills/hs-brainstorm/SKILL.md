...
2. **Delegate a quick scouting pass** (see `.agents/agents/hs-scout.md`)
...
`planning`, `building`, `verifying`, `shipped`) — each skill updates its own
row when it acts. It's a glance-level index, not a duplicate of the detail
already in each spec's own files.
