...
Both are generated for all four supported agents from this file — see
"Per-agent wiring" in each section below. Editing this file and running
`npm exec -- hs agents` is the single source of truth; don't hand-edit any
generated agent file directly, it's overwritten on the next generate.
