...
Resolve one stable target, run two independent assessments, synthesize a design critique, and ask the user what to improve next. The chat response is the deliverable.
...
Evaluate:
- **AI slop**: Would someone believe "AI made this" immediately? Check all DON'T guidance from `SKILL.md`.
...
### Assessment B: Browser Evidence (no bundled detector)

Assessment B is mandatory and must remain isolated from Assessment A until both are complete.

Deterministic scan: the bundled detector is not available in this kit. Report "deterministic scan unavailable" and continue with browser and manual review.

- Manual scan: read the target's markup and styles for the anti-patterns in `SKILL.md` (absolute bans, gradient text, side-stripe borders, glassmorphism, identical card grids, eyebrow / numbered-section scaffolding, low contrast, text overflow) and record file locations.
- Browser evidence is required for a viewable target when browser automation is available. Use a localhost dev/static URL for local files; avoid `file://` unless the available browser explicitly supports this workflow. Create a fresh tab, navigate, and capture screenshots at mobile, tablet, and desktop widths. Read each screenshot back into the conversation; a screenshot you did not read does not count. For multi-view targets, inspect 3-5 representative pages.
- If browser automation is unavailable, say so and report the fallback signal (manual review of source only).

Return: manual scan findings with file locations, browser observations per viewport, false positives, and skipped/failed browser steps with concrete reasons.
...
**Deterministic scan**: State that the bundled detector is unavailable in this kit. Summarize what the manual scan and browser evidence found, with counts and file locations, and flag any false positives.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `CLAUDE.md` contains a `## Design Context` section from `setup`, also generate 1-2 project-specific personas from the audience/brand info.
...
If `CLAUDE.md` contains a `## Design Context` section (generated by `setup`), derive 1–2 additional personas from the audience and brand information:
